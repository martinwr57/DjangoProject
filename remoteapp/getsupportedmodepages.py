...
def getmodepages(pd, dellinq_file, mode_page_type):
    ...
    if mode_page_type == 0x00 or mode_page_type == 0:   
        ...
        ret = pd.send_scsi_passthru(scsi_pt, print_flag = True)
...
def drive_mp(type):
    ...
    for ctrl in controllers: #inventory.ctrl_list:
        ...
        for pd in pd_list:
            for p in cmd_list: 
                    ret, modepages = pd.get_supported_mode_pages()
                    serial_number = pd.spc_props.unit_serial_number
                    dellfile = name_builder(pd, p)
                    dellinq_file = dellfile[0]
                    mode = dellfile[1]
                    print mode
                    key = str(serial_number) + '_' + mode
                    data = getmodepages(pd, dellinq_file, p)
                    if mode == '0x00':
                        print key
                        print data
                    output[key] = data
                ...
                #    print modepage
                #print "Saved"
                #ret, modepages = str dev.get_supported_mode_pages(0xC0)
                #for modepage in modepages:
                #    print modepage
                #print "Default"
                ...
                #    print modepage
    if type == 'CheckIn':
        remote_client_db.mode_pages('CheckIn', output)
    elif type == 'CheckOut':
        remote_client_db.mode_pages('CheckOut', output)
